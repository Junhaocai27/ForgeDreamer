...
class PivotalTuningDatasetCapationPromptOnly(Dataset):
    ...
    def __init__(
        ...
        dataset_size: int = 10, # Renamed from noise_count to avoid confusion; num_samples would also work
    ):
        ...
        if captions is not None:
            self.captions = [str(c).strip() for c in captions] # Ensure string type and strip whitespace
            if not self.captions: # If provided captions list is empty
            ...
            if caption_path.exists():
                with open(caption_path, 'r', encoding='utf-8') as f: # Specify encoding
                    self.captions = [line.strip() for line in f.readlines() if line.strip()] # Strip empty lines
    ...
    def _tokenize_text(self, tokenizer, text, padding_strategy="do_not_pad"):
        # Encapsulates tokenize logic for easy reuse and padding strategy changes
        ...
        return tokenizer(
            ...
            return_tensors=None, # Don't convert to tensor yet; handle in collate_fn
        ).input_ids

    def __getitem__(self, index):
        example = {}
        # Get the raw text for the current index
        # Use index % len(self.captions) to safely cycle through captions,
        # even if self.dataset_size > len(self.captions) (theoretically handled during init)
        ...
        # You may also want to return the raw text for debugging
...
class PivotalTuningDatasetCapationLoraGenerated(Dataset):
    ...
    It also adds the ability to save generated images to disk for debugging and inspection.
    ...
    def __init__(
        ...
        # --- Image transform parameters (applied to images fed to the model) ---
        ...
        if not self.captions_list: # Fallback if still empty
             self.captions_list = [f"default_prompt_{i}" for i in range(self.dataset_size if self.dataset_size > 0 else 10)]

        if self.use_template is not None:
            if 'TEMPLATE_MAP' not in globals() or not isinstance(TEMPLATE_MAP, dict) or use_template not in TEMPLATE_MAP:
                # Fallback if TEMPLATE_MAP is not globally defined or key is missing
                print(f"Warning: TEMPLATE_MAP is not defined or does not contain key '{use_template}'. Falling back to basic prompts.")
                self.templates = None # Indicate no templates available
                self.use_template = None # Disable template usage
            else:
                self.templates = TEMPLATE_MAP[use_template]
                if not self.templates:
                    print(f"Warning: No templates found for '{use_template}' in TEMPLATE_MAP. Falling back to basic prompts.")
                    self.use_template = None # Disable template usage

        self._length = self.dataset_size

        # --- Image transforms (applied to images fed to the model) ---
        self.h_flip = h_flip
        transform_list = []
    ...
    def _clean_filename_prompt(self, prompt_text, max_len=50):
        """Sanitize prompt text so it is suitable as part of a filename."""
    ...
    def __getitem__(self, index):
        ...
        if self.use_template and self.templates: # Check if templates are available
            if self.token_map is None or not self.token_map:
                ...
                print("Warning: A non-empty token_map must be provided when use_template is set. Falling back to base text.")
        ...
        # --- 4. Save the raw generated image (if a path is specified) ---
        #    This step happens before any image transforms
        if self.save_generated_images_path:
            ...
            # Note: the image saved here is the raw generated image, before any transforms (including flips for training)
            filename_parts = [
                self.save_image_prefix,
                f"{index:04d}", # Zero-padded index for easy sorting
            ]
            if prompt_suffix:
            ...
            try:
                ...
                # print(f"Saved raw image: {full_save_path}") # Uncomment if needed
            except Exception as e:
                print(f"Error: Could not save raw image to {full_save_path}: {e}")
                
            torch.cuda.empty_cache()
            # Optionally print GPU memory again to check effect of cache clear
        ...
        # --- 5. Apply image transforms to get the final tensor for training ---
        #    These transforms prepare the image for model input and do not affect the already-saved raw image
        instance_image_tensor = self.image_transforms(generated_image_pil_original) # Tensor range [-1, 1]

        # --- 6. (Optional) Apply horizontal flip to the training image tensor ---
        if self.h_flip and random.random() > 0.5:
            hflip_transform = transforms.RandomHorizontalFlip(p=1.0)
            instance_image_tensor = hflip_transform(instance_image_tensor)
            # Note: this flip only applies to the tensor fed to the model;
            # the saved image `generated_image_pil_original` is not flipped.
            # If you want the filename to reflect this, more complex logic or saving the transformed image would be needed.

        example["instance_images"] = instance_image_tensor

        # --- 7. Tokenize the prompt text ---
