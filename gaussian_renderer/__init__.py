...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, black_video = False,
           override_color = None, sh_deg_aug_ratio = 1.0, bg_aug_ratio = 0.3, shs_aug_ratio=1.0, scale_aug_ratio=1.0, test = False):
    # change sh_deg_aug_ratio to 1.0 to disable SH degree augmentation
    # sh_deg_aug_ratio_default = 0.1
    ...
    rendered_image, radii, depth_alpha = rasterizer(   # memory increase 4GiB
        means3D = means3D,
        means2D = means2D,
        shs = shs,
