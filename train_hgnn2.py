...
from model.MTC import *
from model.ATT import *
from model.MLP import *
from model.MLP_new import *
from Hyper3DG.DINO_Ex import *
from Hyper3DG.hgnn_3D import *
from Hyper3DG.k_means import *
from Hyper3DG.HG import *
from Hyper3DG.GS_Recover import *
...
# def create_text_graph(text, view=None):
...
#     node_features = torch.eye(len(nodes))  # 每个单词一个独热向量
...
# def create_text_graph(text, view=None):
#     from conceptnet_lite import get_concept
#     nodes = []
#     edges = []
...
#         concept = get_concept(token.text)  # 从 ConceptNet 中获取概念
#         if concept:
#             for edge in concept.edges:
#                 if edge.end.label not in nodes:
#                     nodes.append(edge.end.label)
#                 edges.append((nodes.index(token.text), nodes.index(edge.end.label)))

#     # 转化为 PyTorch Geometric 的图结构
#     edge_index = torch.tensor(edges).t().contiguous()
#     node_features = torch.eye(len(nodes))  # 或结合嵌入
#     graph = Data(x=node_features, edge_index=edge_index)
...
#     model = GCN(input_dim=graph.x.shape[1], hidden_dim=128, output_dim=1024).to(guidance.device)
#     graph_embed = model(graph.x.to(guidance.device), graph.edge_index.to(guidance.device)).mean(dim=0)
#     embeddings['default'] = graph_embed.unsqueeze(0).repeat(77, 1).unsqueeze(0)  # (1, 77, 512)

#     # 2. Negative 图嵌入
#     graph = create_text_graph(guidance_opt.negative)
#     model = GCN(input_dim=graph.x.shape[1], hidden_dim=128, output_dim=1024).to(guidance.device)
#     graph_embed = model(graph.x.to(guidance.device), graph.edge_index.to(guidance.device)).mean(dim=0)
#     embeddings['uncond'] = graph_embed.unsqueeze(0).repeat(77, 1).unsqueeze(0)  # (1, 77, 512)
...
#         graph_embed = model(graph.x.to(guidance.device), graph.edge_index.to(guidance.device)).mean(dim=0)
#         # 调整为 CLIP 嵌入格式
#         embeddings[view] = graph_embed.unsqueeze(0).repeat(77, 1).unsqueeze(0)  # (1, 77, 512)

#     # 4. Inverse text 图嵌入
#     graph = create_text_graph(guidance_opt.inverse_text)
#     model = GCN(input_dim=graph.x.shape[1], hidden_dim=128, output_dim=1024).to(guidance.device)
#     graph_embed = model(graph.x.to(guidance.device), graph.edge_index.to(guidance.device)).mean(dim=0)
#     embeddings['inverse_text'] = graph_embed.unsqueeze(0).repeat(77, 1).unsqueeze(0)  # (1, 77, 512)
...
def new_prepare_embeddings(guidance_opt, guidance):
    ...
    def generate_graph_embed(graph, model):
        # 初始化嵌入
        x = graph.x.to(guidance.device)
        edge_index = graph.edge_index.to(guidance.device)
    ...
    gcn_models = {
        'text': GCN(input_dim=create_text_graph(guidance_opt.text).x.shape[1], 
                    hidden_dim=128, 
                    output_dim=1024).to(guidance.device),
...
def guidance_setup(guidance_opt):
    ...
    embeddings2 = prepare_embeddings(guidance_opt, guidance)
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    ...
    progress_bar = tqdm(range(first_iter, opt.iterationsN_0 + opt.iterations), desc="Training progress")
    first_iter += 1
    ...
    total_loss = 0
    total_iterations = opt.iterationsN_0 + opt.iterations * opt.iterationsN_1 + 1

    for iteration in tqdm(range(first_iter, total_iterations), desc="训练进度", unit="step"):      
        ...
        if iteration >= opt.iterationsN_0 + 1 and iteration < opt.iterationsN_0 + opt.iterations * opt.iterationsN_1 + 1:
            for i in range(C_batch_size):
                # HGRefiner
                # 计算当前循环次数和总进度
                current_iter = (iteration - opt.iterationsN_0 - 1) // opt.iterationsN_1 + 1
                total_iters = opt.iterations
                
                # 显示总体进度
                print(f"\n{'='*50}")
                print(f"HGRefiner 总体进度: 第 {current_iter} 轮,共 {total_iters} 轮")
                print(f"{'='*50}")
                
                # 显示当前轮次内的进度
                current_inner_iter = (iteration - opt.iterationsN_0 - 1) % opt.iterationsN_1 + 1
                print(f"当前轮次内进度: {current_inner_iter}/{opt.iterationsN_1}")
                
                if (iteration - opt.iterationsN_0 - 1) % opt.iterationsN_1 == 0:
                    print("\n开始新的优化轮次...")
                    hg_flag = False
                
                # 显示patch处理进度
                print("\n1. 处理3DGS patches...")
                patch_processor = Patch3DGS()
                patch_list, point_to_patch, centers = patch_processor.split_3dgs(gaussians, opt.kmeans_patch_num)
                print(f"-> 成功创建 {len(patch_list)} 个patches")
                
                # 显示渲染进度
                print("\n2. 渲染patches...")
                for i, patch in enumerate(tqdm(patch_list, desc="渲染进度")):
                    patch_pkg = render(viewpoint_cam, patch, pipe, background,
                    ...
                    patch_image = patch_pkg["render"]
                    patch_images.append(patch_image)
                
            print("\n4. 提取DINO特征...")
            extractor = DINOFeatureExtractor()
            l_patch_features = extractor.extract_features(patch_images)

            # 1. 生成索引矩阵 [50, 4]
            patch_indices = torch.arange(50).unsqueeze(1) + torch.tensor([0, 50, 100, 150])  # [50, 4]

            # 2. 根据索引获取对应的 4 个视角数据
            patch_data = l_patch_features[patch_indices]  # 形状 [50, 4, 384]

            # 3. 进行合并（示例：求均值）
            merged_tensor = patch_data.mean(dim=1)  # [50, 384]
            ...
            # 获取各个特征的维度
            xyz_dim = first_patch._xyz.shape[-1]  # 3
            scaling_dim = first_patch._scaling.shape[-1]  # 3
            rotation_dim = first_patch._rotation.shape[-1]  # 4
            opacity_dim = first_patch._opacity.shape[-1]  # 1
            features_dc = first_patch._features_dc.shape[-1]
            features_rest = first_patch._features_rest.shape[-1] if first_patch._features_rest.numel() > 0 else 0
            feature_dim = features_dc + features_rest

            # 创建tensor存储所有特征
            total_dim = xyz_dim + scaling_dim + rotation_dim + opacity_dim + feature_dim
            merged_features = torch.zeros((num_patches, total_dim), device=first_patch._xyz.device)

            print("-> 合并patch特征...")
            for i, patch in enumerate(tqdm(patch_list, desc="特征合并进度")):
                # 直接访问tensor属性计算平均值
                xyz = patch._xyz.mean(dim=0).clone().detach() # 3
                scaling = patch._scaling.mean(dim=0).clone().detach() # 3
                rotation = patch._rotation.mean(dim=0).clone().detach() # 4
                opacity = patch._opacity.mean(dim=0).clone().detach() # 1
                
                # 合并features_dc和features_rest
                features_dc = patch._features_dc.mean(dim=0).clone().detach()
                if patch._features_rest.numel() > 0:
                    features_rest = patch._features_rest.mean(dim=0).clone().detach()
                    features = torch.cat([features_dc, features_rest])
                else:
                    features = features_dc
                
                # 按顺序填充
                current_idx = 0
                merged_features[i, current_idx:current_idx + xyz_dim] = xyz
                current_idx += xyz_dim
                merged_features[i, current_idx:current_idx + scaling_dim] = scaling
                current_idx += scaling_dim
                merged_features[i, current_idx:current_idx + rotation_dim] = rotation
                current_idx += rotation_dim
                merged_features[i, current_idx:current_idx + opacity_dim] = opacity
                current_idx += opacity_dim
                merged_features[i, current_idx:] = features

            averaged_3DGS_features = merged_features
            
            patch_3DGS_repre = torch.cat((averaged_3DGS_features, merged_tensor), dim=1)
            
            print("\n5. 初始化超图网络...")
            # 初始化超图网络和优化器
            HG_spa = HypergraphBuilder(opt.Klat)
            HG_lat = HypergraphBuilder(opt.Kspa)
            net = HGNN_3D(in_channels=patch_3DGS_repre.shape[1], hidden_channels=256)
            ...
            if hg_flag == False:
                print("\n6. 构建超图结构...")
                # 构建空间和语义超图
                HG_spa.build_hypergraph(averaged_3DGS_features)
                HG_lat.build_hypergraph(merged_tensor)
                
                # 获取各自的H矩阵
                H_spa = HG_spa.get_H_matrix()
                H_lat = HG_lat.get_H_matrix()
                
                # 将两个H矩阵在超边维度上拼接
                H_all = torch.cat((H_spa, H_lat), dim=-1)
                
                # 获取H矩阵的稀疏表示
                indices = H_all._indices()  # 获取非零元素的索引
                values = H_all._values()   # 获取非零元素的值
                
                # 构建边列表(e_list)
                e_list = []
                for i in range(H_all.shape[1]):  # 遍历每条超边
                    # 找到当前超边连接的所有顶点
                    edge_vertices = indices[0][indices[1] == i]
                    if len(edge_vertices) > 0:  # 只添加非空超边
                        e_list.append(edge_vertices.tolist())
                
                # 使用边列表构建新的超图
                HG_all = Hypergraph(
                    num_v=patch_3DGS_repre.size(0),  # 顶点数量
                    e_list=e_list,                   # 边列表
                    e_weight=[1.0]*len(e_list)       # 超边权重
                )
                hg_flag = True
                print("-> 超图构建完成")
            ...
            # 前向传播
            refined_features = net(patch_3DGS_repre, HG_all)
            refined_features.to(device)

            # 计算重建损失
            # reconstruction_loss = F.mse_loss(refined_features, patch_3DGS_repre)
                
            # 添加正则化损失
            # l2_reg = torch.tensor(0., device=device)
            # for param in net.parameters():
            #     l2_reg += torch.norm(param)
            # total_loss = reconstruction_loss + 0.01 * l2_reg
            ...
            # print(f"-> 当前损失: {total_loss.item():.6f}")
            
            print("\n9. 恢复3DGS特征...")
            # 使用GaussianRecovery恢复3DGS特征
            recover = GaussianRecovery()
            updated_3DGS = recover.recover_3DGS(refined_features, averaged_3DGS_features, gaussians, point_to_patch, total_dim, opt.kmeans_patch_num)
            gaussians = recover.update_gaussians(gaussians, updated_3DGS)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--opt', type=str, default='/home/s414e2/CJH/Text-to-3D/LucidDreamer/configs/bolt_hgnn.yaml')
    ...
    # 分两个阶段计算迭代点
    test_iter_0 = [1] + [k * op.iterationsN_0 // args.test_ratio for k in range(1, args.test_ratio)] + [op.iterationsN_0]
    test_iter_1 = [op.iterationsN_0 + k * op.iterationsN_1 // args.test_ratio for k in range(1, args.test_ratio)] + [op.iterationsN_0 + op.iterationsN_1]
    args.test_iterations = test_iter_0 + test_iter_1

    save_iter_0 = [k * op.iterationsN_0 // args.save_ratio for k in range(1, args.save_ratio)] + [op.iterationsN_0]
    save_iter_1 = [op.iterationsN_0 + k * op.iterationsN_1 // args.save_ratio for k in range(1, args.save_ratio)] + [op.iterationsN_0 + op.iterationsN_1]
    args.save_iterations = save_iter_0 + save_iter_1
