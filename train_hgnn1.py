...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    ...
    for iteration in range(first_iter, opt.iterationsN_0 + opt.iterations + 1):        
        ...
        for i in range(C_batch_size):
            ...
            # Render
            if iteration < opt.iterationsN_0 + 1:
            ...
            # HGRefiner
            elif iteration >= opt.iterationsN_0 + 1 and iteration < opt.iterationsN_0 + opt.iterations * opt.iterationsN_1 + 1:
                # 计算当前循环次数和总进度
                ...
                patch_3DGS_repre = torch.cat((averaged_3DGS_features, l_patch_features), dim=1)
                ...
                if hg_flag == False:
                    ...
                    HG_lat.build_hypergraph(l_patch_features)
                ...
                updated_3DGS = recover.recover_3DGS(refined_features.detach(), averaged_3DGS_features, gaussians, point_to_patch, total_dim, opt.kmeans_patch_num)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--opt', type=str, default='/home/s414e2/CJH/Text-to-3D/LucidDreamer/configs/bolt.yaml')
