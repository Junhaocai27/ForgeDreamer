...
class StableDiffusion(nn.Module):
    ...
    def train_step_perpneg(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
        ...
        if iteration % guidance_opt.vis_interval == 0:
            ...
            with torch.no_grad():
                ...
                dino_feature_loss = self.compute_dino_feature_mse_loss(
                    pred_x0_pos, 
    ...
    def compute_dino_feature_mse_loss(self, pred_x0_pos, updated_features_list, iteration):
        """
        计算pred_x0_pos的DINO特征与更新后pred_rgb特征的MSE损失
        ...
        try:
            ...
            if iteration % 100 == 0:
                print(f"[INFO] DINO feature MSE loss: {mse_loss.item():.6f}")
            ...
            print(f"[ERROR] Failed to compute DINO feature MSE loss: {e}")
            import traceback
            traceback.print_exc()
            return torch.tensor(0.0, device=self.device, requires_grad=True)
    ...
    def extract_and_compute_similarity(self, pred_rgb):
        """
        提取特征并计算相似度，然后应用超图卷积的主函数
        
        Args:
            pred_rgb: 当前的pred_rgb张量 [B, 3, H, W]
        """
        self.call_count += 1
        ...
        if len(self.pred_rgb_queue) == 4:
            # 🔥 控制详细输出的频率
            verbose = self.call_count % 100 == 0  # 每100次才详细输出
            
            if verbose:
                print(f"[INFO] Processing similarity computation at call {self.call_count}")
            ...
            for i, rgb in enumerate(self.pred_rgb_queue):
                ...
                if verbose:
                    print(f"[INFO] Extracted feature {i+1}/4, shape: {feature.shape}")
            ...
            updated_features_list = features_list  # 默认使用原始特征
            
            if hyperedges:
                if verbose:
                    print(f"[INFO] Found {len(hyperedges)} hyperedges, applying hypergraph convolution...")
                
                # 应用超图卷积更新特征
                try:
                    updated_features_list = self.apply_hypergraph_convolution_verbose(features_list, hyperedges, verbose)
                ...
                if verbose:
                    print("[WARNING] No hyperedges found, skipping hypergraph convolution")
            
            # 🔥 可视化保存（每200轮）
            VISUALIZATION_INTERVAL = 200
            if self.call_count % VISUALIZATION_INTERVAL == 0:
                try:
                    with torch.no_grad():
                        latest_pred_rgb = self.pred_rgb_queue[-1].detach()
                        print(f"[INFO] Starting feature visualization for iteration {self.call_count}")
                        self.visualize_and_save_features(latest_pred_rgb, updated_features_list, self.call_count)
                        print(f"[INFO] Feature visualization completed for iteration {self.call_count}")
                except Exception as e:
            ...
            if self.call_count % 200 == 0:
                try:
                    self.save_features_summary(self.call_count)
                    self.save_hypergraph_analysis(self.call_count)
                except Exception as e:
                    print(f"[WARNING] Saving analysis failed: {e}")
            
            if verbose:
                print(f"[INFO] Similarity computation and hypergraph convolution completed. Total results: {len(self.similarity_results)}")
