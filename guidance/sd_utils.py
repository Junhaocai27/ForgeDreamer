...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16, vram_O, t_range=[0.02, 0.98], max_t_range=0.98, num_train_timesteps=None, 
        ...
        if is_safe_tensor:
            pipe = StableDiffusionPipeline.from_single_file('/root/LucidDreamer/stable-diffusion-2-1-base', use_safetensors=True, torch_dtype=self.precision_t, load_safety_checker=False)
        else:
            pipe = StableDiffusionPipeline.from_pretrained('/root/LucidDreamer/stable-diffusion-2-1-base', torch_dtype=self.precision_t, local_files_only=True)

        self.ism = not guidance_opt.sds
        self.scheduler = DDIMScheduler.from_pretrained('/root/LucidDreamer/stable-diffusion-2-1-base' if not is_safe_tensor else base_model_key, subfolder="scheduler", torch_dtype=self.precision_t)
    ...
    def extract_edge_attention_mask(self, pred_rgb, save_folder=None, iteration=0, vis_interval=50):
        ...
        if not hasattr(self, 'dino_model'):
            try:
                ...
                # 加载预训练权重
                pretrained_weights = '/root/LucidDreamer/Hyper3DG/dino_deitsmall8_pretrain.pth'
    ...
    def train_step_perpneg(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
        ...
        # 计算基于迭代的衰减权重
        total_iterations = getattr(guidance_opt, 'iterations', 5000)  # 总训练迭代次数
        uncond_start_weight = getattr(guidance_opt, 'uncond_start_weight', 1.0)  # 起始权重
        uncond_end_weight = getattr(guidance_opt, 'uncond_end_weight', 0.1)  # 结束权重

        # 线性衰减权重计算
        progress = min(iteration / total_iterations, 1.0)
        uncond_weight = uncond_start_weight - progress * (uncond_start_weight - uncond_end_weight)

        # 使用计算得到的权重
        # grad = w(self.alphas[t]) * (pred_noise - target) + uncond_weight * (noise_pred_uncond - target)
        grad = w(self.alphas[t]) * (pred_noise - target)

        # 输出当前使用的权重值（可选，便于调试）
        if iteration % 100 == 0:
            print(f"[迭代 {iteration}] 非条件项权重: {uncond_weight:.4f}")

        # 确定是否应用低通滤波掩码
        lowpass_start_iter = getattr(guidance_opt, 'low_pass_start_iter', 0)  # 默认700次迭代后开始
        
        # 计算梯度
        grad = w(self.alphas[t]) * (pred_noise - target) + (noise_pred_uncond - target)
