...
from select_lora import select_lora_for_text, find_matching_lora
...
def create_text_graph(text, view=None, model_name="/root/LucidDreamer/bert-base-uncased/bert-base-uncased"):
...
def guidance_setup(guidance_opt):
    ...
    # 找到匹配的 LoRA 文件
    # lora_paths_with_concepts = select_lora_for_text(guidance_opt.text)
    
    # if lora_paths_with_concepts:
    #     # 使用找到的 LoRA 文件
    #     LoRA_path = lora_paths_with_concepts
    # else:
    #     # 使用默认的 LoRA 文件
    #     LoRA_path = ""
    ...
    if guidance_opt.guidance=="SD":
        ...
        guidance = StableDiffusion(guidance_opt.g_device, guidance_opt.fp16, guidance_opt.vram_O, 
                                   ...
                                   LoRA_path = guidance_opt.LoRA_path, # 初始不加载LoRA
    ...
    # 1. 获取图嵌入（只需获取一次，所有视角共享）
    graph_embeddings = new_prepare_embeddings(guidance_opt, guidance)
    graph_embeddings = {k: v * 0.1 for k, v in graph_embeddings.items()}  # 0.1倍图嵌入
    
    # 2. 获取视角特定的文本嵌入
    view_embeddings = prepare_embeddings(guidance_opt, guidance)
    
    # 3. 创建视角特定的图嵌入结构
    graph_embeddings_view = {
        'front': {},
        'up': {}
    }
    
    # 复制图嵌入到两个视角特定的字典中
    for key in graph_embeddings:
        graph_embeddings_view['front'][key] = graph_embeddings[key]
        graph_embeddings_view['up'][key] = graph_embeddings[key]
    ...
    # 5. 为front和up视角分别融合图嵌入和文本嵌入
    for view_type in ['front', 'up']:
        final_view_dict = {}
        
        # 复制当前视角类型的所有文本嵌入
        for key in view_embeddings[view_type]:
            final_view_dict[key] = view_embeddings[view_type][key]
            
        # 如果图嵌入中有相应的键，添加到文本嵌入中
        for key in graph_embeddings_view[view_type]:
            if key in final_view_dict:
                final_view_dict[key] = final_view_dict[key] + graph_embeddings_view[view_type][key]
        
        # 将处理好的嵌入集合存入最终字典
        final_embeddings[view_type] = final_view_dict
    
    return guidance, final_embeddings
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        text_z_inverse_front = torch.cat([embeddings['front']['uncond'], embeddings['front']['inverse_text']], dim=0)
        text_z_inverse_up = torch.cat([embeddings['up']['uncond'], embeddings['up']['inverse_text']], dim=0)
        ...
        for i in range(C_batch_size):
            ...
            if polar != 0:  # 非零表示45度视角，使用up触发词
                current_embeddings = embeddings['up']
                view_type = "up"
                text_z_inverse = text_z_inverse_up
                
                # up_lora_path = find_matching_lora(guidance_opt.text, "up")
                
                # guidance.update_lora(up_lora_path)
            else:  # 为零表示正面视角，使用front触发词
                current_embeddings = embeddings['front']
                view_type = "front"
                text_z_inverse = text_z_inverse_front
                
                # front_lora_path = find_matching_lora(guidance_opt.text, "front")
                
                # guidance.update_lora(front_lora_path)

            # 记录当前样本使用的视角类型
            view_types.append(view_type)
            
            # if i == 0 and iteration % 100 == 0:
            #     print(f"[ITER {iteration}] 相机角度(方位={azimuth:.1f}°, 倾斜={polar:.1f}°) -> 使用{view_type}触发词")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--opt', type=str, default='/home/s414e2/CJH/Text-to-3D/LucidDreamer/configs/new_experiment/screw/screw12.yaml')
