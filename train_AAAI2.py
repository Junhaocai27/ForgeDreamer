...
def guidance_setup_enhanced(guidance_opt):
    ...
    if guidance_opt.guidance=="SD":
        from guidance.sd_utils_copy_cjh import StableDiffusion
...
def get_mixed_viewpoint_stack(scene, iteration, opt):
    """
    根据训练进度动态调整正面视角和45度视角的比例
    ...
    Returns:
        mixed_viewpoint_stack: 混合的视角栈
        current_ratio_info: 当前比例信息
    """
    # 计算训练进度 (0.0 到 1.0)
    progress = min(iteration / opt.iterations, 1.0)
    
    # 🔥 定义视角比例变化曲线
    # 前期: 正面视角90%，45度视角10%
    # 后期: 正面视角50%，45度视角50%
    front_ratio_start = 0.9  # 起始正面视角比例
    front_ratio_end = 0.5    # 结束正面视角比例
    
    # 线性插值计算当前正面视角比例
    current_front_ratio = front_ratio_start - progress * (front_ratio_start - front_ratio_end)
    current_45_ratio = 1.0 - current_front_ratio
    
    # 生成视角
    total_cameras = 120
    front_count = int(total_cameras * current_front_ratio)
    deg45_count = total_cameras - front_count
    
    # 获取两种视角的相机
    front_cameras = scene.getCircleVideoCameras(batch_size=front_count, render45=False)
    deg45_cameras = scene.getCircleVideoCameras(batch_size=deg45_count, render45=True)
    
    # 混合并打乱顺序
    mixed_cameras = front_cameras + deg45_cameras
    random.shuffle(mixed_cameras)  # 打乱顺序，避免连续同类型视角
    
    # 返回比例信息用于调试
    current_ratio_info = {
        'iteration': iteration,
        'progress': progress,
        'front_ratio': current_front_ratio,
        'deg45_ratio': current_45_ratio,
        'front_count': front_count,
        'deg45_count': deg45_count,
        'total_count': total_cameras
    }
    
    return mixed_cameras, current_ratio_info
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        #         viewpoint_stack = scene.getCircleVideoCameras(batch_size=120, render45=False).copy()
        #         render45_mode = False
        #     else:  # 训练后期使用45度倾斜视角
        #         viewpoint_stack = scene.getCircleVideoCameras(render45=True).copy()
        #         render45_mode = True 
        if not viewpoint_stack:
            viewpoint_stack, current_ratio_info = get_mixed_viewpoint_stack(scene, iteration, opt)
        ...
        for i in range(C_batch_size):
            ...
            if not viewpoint_stack or len(viewpoint_stack) == 0:
                ...
                # 打印调试信息
                if iteration % 500 == 0:
                    print(f"[CAMERA] Iter {iteration}: Refreshed viewpoint stack")
                    print(f"  Front ratio: {current_ratio_info['front_ratio']:.2f}")
                    print(f"  45deg ratio: {current_ratio_info['deg45_ratio']:.2f}")
                    print(f"  Stack size: {len(viewpoint_stack)}")
            
            try:
                viewpoint_cam = viewpoint_stack.pop()
            except IndexError:
                # 🔥 如果还是空，立即重新生成（不回退到随机）
                print(f"[WARNING] Viewpoint stack empty at iter {iteration}, regenerating...")
                viewpoint_stack, current_ratio_info = get_mixed_viewpoint_stack(scene, iteration, opt)
                viewpoint_cam = viewpoint_stack.pop()
